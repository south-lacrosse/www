...
class Monitor_Command {
	...
	public function posts($args, $assoc_args) {
		...
		$assoc_args = array_merge( [
			'fields' =>
				'ID,post_title,post_type,post_status,post_name,post_modified_gmt,post_author,modified_author',
			'format' => 'table',
		], $assoc_args );
		...
		&& str_contains($assoc_args['fields'], 'modified_author')) {
			$select = ', meta_value AS modified_author';
			$from = "LEFT OUTER JOIN $wpdb->postmeta ON post_id = ID and meta_key = '_edit_last'";
		}
		$rows = $wpdb->get_results($wpdb->prepare(
			"SELECT ID, post_title, post_type, post_status, post_name, post_modified_gmt, post_author$select
