...
(function () {
  'use strict';
  var menu, mainMenu, closeMenu, showMenuBtn, shownDropdown, dropdownUl,
    lastMenuLink, passive = false, pulloutTimeout, isPulloutMenuOpen = false,
    addedListeners = false, mpTabIndex = 0, doc = document, win = window;
    ...
    if (shownDropdown) {
      ...
      if (!addedListeners) {
        ...
        doc.getElementById('main-menu')
          .addEventListener('mouseover', mouseOverMainMenu, false);
  ...
  function docFocus(event) {
    ...
    if (target.className.substring(0,3) != 'ma '
